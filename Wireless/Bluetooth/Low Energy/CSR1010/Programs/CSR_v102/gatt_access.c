...
#include "bluepay_service.h"/* Battery Service interface */
#include "bluepay_service_uuids.h"  /* Battery Service UUIDs */
...
#define TX_POWER_VALUE_LENGTH                             (2) //Danil (2)
...
extern void HandleAccessRead(GATT_ACCESS_IND_T *p_ind)
{
    ...
    else if(BluePayCheckHandleRange(p_ind->handle))
    {
        /* Attribute handle belongs to BATTERY service */
        BluePayHandleAccessRead(p_ind);
    }
    else
    {
...
{   
    ...
    {
        /* Attribute handle belongs to BATTERY service */
        BluePayHandleAccessWrite(p_ind);
    }
    else
    {
...
{
    //DebugIfWriteString("GattStartAdverts\r\n");
    if(!BleEnableFlag) return;
...
extern uint16 GetSupported16BitUUIDServiceList(uint8 *p_service_uuid_ad)
{
    ...
    p_service_uuid_ad[size_data++] = WORD_LSB(UUID_BLUEPAY_SERVICE);
    p_service_uuid_ad[size_data++] = WORD_MSB(UUID_BLUEPAY_SERVICE);
